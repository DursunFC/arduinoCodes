...
const int buzzer1Pin = 6; // Buzzer 1 pin
const int buzzer2Pin = 7; // Buzzer 2 pin
...
void loop() {
  ...
  // Check if AX or AY exceeds the threshold and activate corresponding buzzers
  if (abs(ax) > threshold) {
    tone(buzzer1Pin, 1000); // Buzzer 1 with 1000 Hz tone
  } else {
    noTone(buzzer1Pin);
  }

  if (abs(ay) > threshold) {
    tone(buzzer2Pin, 1500); // Buzzer 2 with 1500 Hz tone
  } else {
    noTone(buzzer2Pin);
  }
